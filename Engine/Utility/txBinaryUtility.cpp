#include "txBinaryUtility.h"
#include "Utility.h"

void txBinaryUtility::swapPixelBuffer(char* buffer, int width, int height, int blockByte)
{
...
void txBinaryUtility::swapColourByte(char* buffer, int width, int height, int blockByte, int exchangeIndex0, int exchangeIndex1)
{
	for (int i = 0; i < height; ++i)
	{
		for (int j = 0; j < width; ++j)
		{
			char temp = buffer[i * width * blockByte + j * blockByte + exchangeIndex0];
			buffer[i * width * blockByte + j * blockByte + exchangeIndex0] = buffer[i * width * blockByte + j * blockByte + exchangeIndex1];
			buffer[i * width * blockByte + j * blockByte + exchangeIndex1] = temp;
		}
	}
}

void txBinaryUtility::upsideDownBuffer(char* buffer, int bufferWidth, int startX, int startY, int width, int height, int blockByte)
{
	for (int i = 0; i < height / 2; ++i)
	{
		for (int j = 0; j < width; ++j)
...
void txBinaryUtility::YUVToRGBA(char* YUVData, int width, int height, char* RGBAData)
{
	char Y, U, V;
	for (int i = 0; i < height; ++i)
	{
...
void txBinaryUtility::RGBAToYUV(char* RGBAData, int width, int height, char* YUVData)
{
	char R, G, B;
	for (int i = 0; i < height; ++i)
	{
...
void txBinaryUtility::YUV420PToRGB32(unsigned char* yuv420[3], unsigned char* rgb32, int width, int height)
{
	int R, G, B, Y, U, V;
	...
	for (int y = 0; y < height; ++y)
	{
		for (int x = 0; x < width; ++x)
		{
			...
			//防止越界
			txMath::clamp(R, 0, 255);
			txMath::clamp(G, 0, 255);
			txMath::clamp(B, 0, 255);
...
int txBinaryUtility::checkCRC(char c)
{
	int count = 0;
...
bool txBinaryUtility::memoryCompare(char* buffer0, char* buffer1, const int& length)
{
