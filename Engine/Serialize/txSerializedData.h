...
struct DataParameter
{
	...
	DataParameter(char* data, const int& dataSize, const std::string& dataType, const std::string& describe)
	{
		mDataPtr = data;
		mDataSize = dataSize;
		mDataType = dataType;
...
public:
	...
	bool readStringList(const txVector<std::string>& dataList);
	...
	const txVector<DataParameter>& getParameterList() const { return mDataParameterList; }
	// 在子类构造中调用
	...
	void addArrayParam(const char* param, const std::string& type, const int& count, const std::string& describe, int paramPos);
	void deleteParam(const int& paramPos);
	void moveParam(const int& paramPos, const int& destPos);
	// 获得数组类型的大小时,是返回单个元素的大小
	static int getTypeSize(const std::string& type)
	{
		auto iterSize = mTypeSize.find(type);
...
public:
	...
	static txMap<std::string, int> mTypeSize;	// 各个基础数据类型的大小,如果是数组,则是单个元素的大小
public:
