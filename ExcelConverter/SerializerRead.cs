...
// 只读缓冲区,用于解析二进制数组
public class SerializerRead : FileUtility
{
	...
	protected bool readCheck(int readLen)
	{
		if (mBuffer == null)
		{
			if (mShowError)
			{
				Console.WriteLine("Error: buffer is NULL! can not read");
			}
			return false;
		}
		if (!canRead(readLen))
		{
			if (mShowError)
			{
				Console.WriteLine("Error: read buffer out of range! cur index : " + mIndex + ", buffer size : " + mBufferSize + ", read length : " + readLen);
			}
			return false;
		}
		return true;
