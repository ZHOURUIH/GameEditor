...
public:
	...
	static VALUE* randomSelect(const myMap<KEY, VALUE>& list)
	{
	...
	// 从一个map中随机选择多个
	template<typename KEY, typename VALUE>
	static void randomSelect(const myMap<KEY, VALUE>& list, int selectCount, myVector<VALUE*>& selectResult)
	{
		uint count = list.size();
		...
		myVector<int>& temp = newIntVector();
	...
	static float inverseLerp(float a, float b, float value) { return (value - a) / (b - a); }
	...
	// 四舍五入
	static int round(float value) { return value > 0.0f ? (int)(value + 0.5f) : (int)(value - 0.5f); }
	...
	template<typename T>
	static void quickSort(myVector<T>& arr, bool ascend = true)
	{
		quickSort(arr.data(), 0, arr.size() - 1, ascend);
	}
	template<typename T>
	static void quickSort(T* arr, int count, bool ascend = true)
	{
		quickSort(arr, 0, count - 1, ascend);
	}
	template<typename T>
	static void quickSort(myVector<T>& arr, int (*compare)(T&, T&), bool ascend = true)
	{
		quickSort(arr.data(), 0, arr.size() - 1, compare, ascend);
	}
	template<typename T>
	static void quickSort(T* arr, int count, int (*compare)(T&, T&), bool ascend = true)
	{
		quickSort(arr, 0, count - 1, compare, ascend);
	}
protected:
	// 不使用函数指针的方法,但是要求T重载了比较运算符
	template<typename T>
	static void quickSort(T* arr, int low, int high, bool ascend = true)
	{
		if (high <= low)
		{
			return;
...
protected:
	...
	static myVector<int>& newIntVector();
	static void destroyIntVector(myVector<int>& vec);
public:
	static constexpr int POW2_COUNT = 513;
	static uint mGreaterPow2[POW2_COUNT];		// 存储比数值数值大的第一个2的次方数,比如获得比5大的第一个数,则是mGreaterPow2[5]
