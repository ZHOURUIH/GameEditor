...
public:
	...
	template<size_t Length>
	static bool removeString(array<char, Length>& str, const char* subString)
	{
		int subPos = 0;
	...
	template<size_t Length>
	static bool removeULLongsString(array<char, Length>& valueArrayString, ullong value)
	{
		ULLONG_STR(valueString, value);
		// 如果value是在最后,则只移除value字符串
		if (endWith(valueArrayString.data(), valueString))
		{
			return removeString(valueArrayString, valueString);
		}
		...
		else
		{
			...
			return removeString(valueArrayString, needRemoveString.data());
		}
	}
	// 从一个ullong数组的字符串中移除指定的value的字符串
	static bool removeULLongsString(char* valueArrayString, uint length, ullong value);
	...
	// 将value添加到一个ullong数组的字符串中
	static void addULLongsString(char* valueArrayString, uint length, ullong value);
	template<size_t Length>
	...
	static void _itoa_s(int value, array<char, Length>& charArray)
	{
		...
		// 优先查表
		if (value >= 0 && value < INT_STRING_COUNT)
		{
		...
		if ((ullong)value > POWER_INT_10[POWER_INT_COUNT - 1])
		{
			ERROR("int value is too large:" + ullongToString((ullong)value));
			return;
		}
	...
	static void _i64toa_s(ullong value, array<char, Length>& charArray)
	{
		...
		if (value > POWER_ULLONG_10[POWER_ULLONG_COUNT - 1])
		{
	...
	static void stringsToString(char* buffer, uint bufferSize, const char** strList, uint stringCount, const char* seperate = ",");
	//-----------------------------------------------------------------------------------------------------------------------------
	// 字符串转换为基础数据类型数组
	//-----------------------------------------------------------------------------------------------------------------------------
	...
	static void appendValueString(array<char, Length>& queryStr, const char* str, bool toUTF8, bool addComma = true)
	{
		...
		if (toUTF8)
		{
			...
			STR_APPEND3(queryStr, "\"", utf8String, end);
	...
	static void appendConditionString(array<char, Length>& condition, const char* col, const char* str, bool toUTF8, const char* relationalOperator = "=", const char* logicalOperator = NULL)
	{
		if (toUTF8)
		{
			char* utf8String = newCharArray(Length);
			ANSIToUTF8(str, utf8String, Length, false);
			STR_APPEND6(condition, col, relationalOperator, "\"", utf8String, "\"", logicalOperator);
			deleteCharArray(utf8String);
		}
		else
		{
	...
	static void appendConditionStringLike(array<char, Length>& condition, const char* col, const char* str, bool toUTF8, const char* logicalOperator = NULL, const char* prev = "%", const char* end = "%")
	{
		if (toUTF8)
		{
			char* utf8String = newCharArray(Length);
			ANSIToUTF8(str, utf8String, Length, false);
			STR_APPEND7(condition, col, " LIKE \"", prev, utf8String, end, "\"", logicalOperator);
			deleteCharArray(utf8String);
		}
		else
		{
	...
	static void appendUpdateString(array<char, Length>& updateInfo, const char* col, const char* str, bool toUTF8, bool addComma = true)
	{
		...
		if (toUTF8)
		{
			char* utf8String = newCharArray(Length);
			ANSIToUTF8(str, utf8String, Length, false);
			STR_APPEND4(updateInfo, col, " = \"", utf8String, end);
			deleteCharArray(utf8String);
		}
		else
		{
...
protected:
	static constexpr int INT_STRING_COUNT = 4097;
	static constexpr int POWER_INT_COUNT = 11;
	static constexpr int POWER_ULLONG_COUNT = 20;
	static string mIntString[INT_STRING_COUNT];
	static const ullong POWER_INT_10[POWER_INT_COUNT];
	static const ullong POWER_ULLONG_10[POWER_ULLONG_COUNT];
