...
// 用于生成二进制文件的
public class Serializer : EditorBase
{
	...
	protected bool writeCheck(int writeLen)
	{
		...
		if (!mWriteFlag)
		{
			logError("error : the buffer is read only, can not write!");
			return false;
		}
		// 如果缓冲区为空,则创建缓冲区
		if (mBuffer == null)
	...
	protected bool readCheck(int readLen)
	{
		...
		if (mWriteFlag)
		{
			logError("error : the buffer is write only, can not read!");
			return false;
		}
		if (mBuffer == null)
		{
			logError("error : buffer is NULL! can not read");
			return false;
		}
		if (mIndex + readLen > mBufferSize)
		{
			logError("error : read buffer out of range! cur index : " + mIndex + ", buffer size : " + mBufferSize + ", read length : " + readLen);
			return false;
		}
		return true;
	}
