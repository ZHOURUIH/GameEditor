...
void SocketNetManager::init(int port, int broadcastPort)
{
	mPort = port;
...
SocketPacket* SocketNetManager::createPacket(SOCKET_PACKET type)
{
	if (type == SP_MAX)
	{
		return NULL;
...
void SocketNetManager::sendMessage(SocketPacket* packet, bool destroyPacketEndSend)
{
	// 将包的数据存入列表
