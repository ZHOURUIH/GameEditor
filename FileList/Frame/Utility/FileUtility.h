...
public:
	...
	static void findFiles(const string& path, myVector<string>& files, bool recursive = true);
	...
	static void openTxtFile(const string& filePath, string& fileContent);
	...
	static string generateFileMD5(const string& fileName, int& fileSize);
	static string generateFileMD5(const string& fileName, char* buffer, const int& bufferSize, int& fileSize);
};
