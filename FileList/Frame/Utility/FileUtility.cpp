...
void FileUtility::findFiles(const string& pathName, myVector<string>& files, bool recursive)
{
	findFiles(pathName, files, NULL, 0, recursive);
}
...
string FileUtility::generateFileMD5(const string& fileName, int& fileSize)
{
	const int MD5_BUFFER_SIZE = 1024 * 8;
	char buffer[MD5_BUFFER_SIZE];
	return generateFileMD5(fileName, buffer, MD5_BUFFER_SIZE, fileSize);
}

string FileUtility::generateFileMD5(const string& fileName, char* buffer, const int& bufferSize, int& fileSize)
{
	FILE* pFile = NULL;
	...
	fclose(pFile);
	return StringUtility::toUpper(md5.finalize().hexdigest());
}