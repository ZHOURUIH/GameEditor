...
void SocketPacketSpeed::execute()
{
	...
	if ((mData[0] == (unsigned char)0xFE && mData[11] == (unsigned char)0xFF))
	{
		...
		if (sum == mData[10])
		{
			...
			memcpy(speedData->mData, mData, speedData->mDataSize);
			speedData->mPower = power;
			speedData->mRotateSpeed = rotateSpeed;
			speedData->mMachineIndex = mData[9];
			mSpeedDataManager->setSpeedData(speedData);
			TRACE_DELETE(speedData);
		}
	}
}
