...
#define EDITOR_CORE_ERROR_DELAY(...)																\
{																									\
	char buffer[2048];																				\
	sprintf_s(buffer, 2048, __VA_ARGS__);															\
	LOGW(__VA_ARGS__);																				\
	...
	char fileLine[256];																				\
	sprintf_s(fileLine, 256, ",file : %s, line : %d", file.c_str(), __LINE__);						\
	EditorCoreUtility::logError(std::string(buffer) + std::string(fileLine), true);					\
}
...
#define EDITOR_CORE_INFO_DELAY(...)																	\
{																									\
	char buffer[2048];																				\
	sprintf_s(buffer, 2048, __VA_ARGS__);															\
	EditorCoreUtility::logInfo(std::string(buffer), true);											\
}
