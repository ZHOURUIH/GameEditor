...
public:
	...
	// 返回第一个元素,并且将该元素移除
	T pop_front(const bool check = true)
	{
#if _DEBUG
		if (check)
		{
	...
	void setData(const Array<Length, T>& values, const int count, bool check = true)
	{
#if _DEBUG
		if (check)
		{
	...
	void setData(const T* values, const int count, bool check = true)
	{
#if _DEBUG
		if (check)
		{
