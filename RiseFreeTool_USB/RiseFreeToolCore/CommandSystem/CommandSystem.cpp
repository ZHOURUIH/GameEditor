...
bool CommandSystem::interruptCommand(Command* cmd)
{
	...
	txVector<DelayCommand>::iterator iter = mCommandBufferProcess.begin();
	txVector<DelayCommand>::iterator iterEnd = mCommandBufferProcess.end();
...
void CommandSystem::pushCommand(Command* cmd, CommandReceiver* cmdReceiver)
{
...
void CommandSystem::pushDelayCommand(Command* cmd, CommandReceiver* cmdReceiver, float delayExecute)
{
	if (cmd == NULL || cmdReceiver == NULL)
	{
		return;
