...
void SystemUtility::getAllProcess(txSet<std::string>& processList)
{
...
bool SystemUtility::launchExecutable(const std::string& path, const std::string& fullName)
{
	// 启动游戏程序
...
void SystemUtility::setRegistryValue(const std::string& path, const std::string& key, const std::string& value)
{
	HKEY hKey;
...
std::string SystemUtility::getRegistryValue(const std::string& path, const std::string& key)
{
	HKEY hKey;
	LONG nError = RegOpenKeyExA(HKEY_CURRENT_USER, path.c_str(), NULL, KEY_ALL_ACCESS, &hKey);
	if (nError != ERROR_SUCCESS)
...
std::string SystemUtility::getDateTimeString()
{
	char date[256];
	SYSTEMTIME time;
	GetSystemTime(&time);
