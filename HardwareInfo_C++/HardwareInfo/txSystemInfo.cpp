...
DWORD WINAPI txSystemInfo::deviceInfoThread(LPVOID lpParameter)
{
	...
	DEVICE_PROPERTY propMainboardType;
	if (systemInfo->WMI_DeviceQuery(DT_MAINBOARD_TYPE, &propMainboardType, 1) > 0)
	{
		systemInfo->mMainboardType = propMainboardType.szProperty;
	}
