...
public:
	...
	void init();
	void destroy(){}
	std::atomic<bool> isDone()				{ return mDone; }
	...
	const std::string& getMainboardType()	{ return mMainboardType; }
...
protected:
	...
	std::string mCPU;
	std::string mBIOS;
	std::string mMainboardType;
	std::string mCurMAC;
	volatile std::atomic<bool> mDone;
};
