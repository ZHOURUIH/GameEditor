...
void StringUtility::removeAll(string& str, char value)
{
	...
	char* tempBuffer = new char[strLength + 1];
...
void StringUtility::stringToULLongs(const string& str, myVector<ullong>& valueList, const char* seperate)
{
	myVector<string> strList;
	split(str.c_str(), seperate, strList);
	FOR_VECTOR_CONST(strList)
...
string StringUtility::UnicodeToANSI(const wchar_t* str)
{
	...
	char* pElementText = new char[allocSize];
...
void StringUtility::appendValueULLong(char* queryStr, uint size, const ullong& value, bool addComma)
{
...
void StringUtility::appendConditionULLong(char* condition, uint size, const char* col, const ullong& value, const char* relationalOperator, const char* logicalOperator)
{
	ULLONG_STR(temp, value);
	STR_APPEND4_N(condition, size, col, relationalOperator, temp, logicalOperator);
}
