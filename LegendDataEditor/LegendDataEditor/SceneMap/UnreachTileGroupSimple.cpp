...
myVector<int> UnreachTileGroupSimple::mTempAddTriangles;

UnreachTileGroupSimple::UnreachTileGroupSimple(int id, MapDataSimple* mapData)
{
	mGroupID = id;
	mMapData = mapData;
...
void UnreachTileGroupSimple::addTile(MapTileSimple* tile)
{
...
void UnreachTileGroupSimple::optimizeUnreach()
{
	// 移除多余的三角形
...
bool UnreachTileGroupSimple::checkPositionValid(const Vector2& pos, int tileIndex)
{
	if (tileIndex == -1)
	{
...
int UnreachTileGroupSimple::getTileIndexInList(int x, int y)
{
	int tileIndex = -1;
...
bool UnreachTileGroupSimple::generateAddTriangle(int x, int y)
{
	if (x < 0 || x >= mMapWidth || y < 0 || y >= mMapHeight)
	{
		return false;
